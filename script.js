window.onload = function () {
  ...
  imageUpload.addEventListener("change", function (e) {
    ...
    reader.onload = function () {
      ...
      textOverlayCod.style.width = image.width + "600px"; //text-cod
      textOverlayCod.style.height = image.height + "px"; //text-cod
      textOverlayCor.style.width = image.width + "600px"; //text-cor
      textOverlayCor.style.height = image.height + "px"; //text-cor
      textOverlaySemJuros.style.width = image.width + "600px"; //text-cor
  ...
  escreverButton.addEventListener("click", function () {
    ...
    textOverlayCod.textContent = "COD: " + textInputCod.value; //text-cod + text-input-cod
    textOverlayCor.textContent = "COR: " + textInputCor.value; //text-cor + text-input-cor
    textOverlaySemJuros.textContent = textInputSemJuros.value + "X SEM JUROS"; //text-semjuros + text-input-semjuros
    textOverlayAvista.textContent = "Av. R$ " + textInputAvista.value; //text-avista + text-input-avista
    textOverlayCartao.textContent = "CARTÃO: " + textInputCartao.value; //text-cartao + text-input-cartao
    textInput.textContent = "VALIDADE:" + textInputValidade.value; //
  });
};
...
btnGenerate.addEventListener("click", () => {
  ...
  const options = {
    ...
    html2canvas: { scale: 2},
    jsDPF: { unit: "mm", format: "a4", orientation: "portrait" },
  };
